...
public class BlockDragonPortal extends BlockBreakable
{
	...
	{
	         this.blockIcon = par1IconRegister.registerIcon("jcdragons:DragonPortal");
	}
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
    {
        ...
        if (par1World.getBlockId(par2 - 1, par3, par4) == JCDMBase.dragonRune.blockID || par1World.getBlockId(par2 + 1, par3, par4) == JCDMBase.dragonRune.blockID)
        {
            b0 = 1;
        }

        if (par1World.getBlockId(par2, par3, par4 - 1) == JCDMBase.dragonRune.blockID || par1World.getBlockId(par2, par3, par4 + 1) == JCDMBase.dragonRune.blockID)
        {
            b1 = 1;
        }
        ...
        else
        {
            ...
            for (l = -1; l <= 2; ++l)
            {
                for (i1 = -1; i1 <= 3; ++i1)
                {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
                    {
                        ...
                        if (flag)
                        {
                            if (j1 != JCDMBase.dragonRune.blockID)
                            {
                                return false;
                            }
                        }
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, JCDMBase.dragonPortal.blockID, 0, 2);
                }
            }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != JCDMBase.dragonRune.blockID)
        {
        ...
        else
        {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == JCDMBase.dragonRune.blockID)
            {
                ...
                {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != JCDMBase.dragonRune.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != JCDMBase.dragonRune.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
                    {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
